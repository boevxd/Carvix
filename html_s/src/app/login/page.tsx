...
export default function LoginPage() {
  ...
  const demoAccounts = [
    ...
    { label: 'Диспетчер', username: 'dispatcher', password: '123456' },
    { label: 'Водитель', username: 'driver1', password: '123456' },
  ];
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-12">
        <div className="w-full max-w-sm">
          ...
          <div className="mt-8 p-4 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200/60 dark:border-slate-700/60">
            ...
            <p className="text-[10px] text-slate-400 mt-3">Учётные записи из десктоп-приложения Carvix</p>
          </div>
        </div>
      </div>
      ...
      <div className="hidden lg:flex flex-1 bg-slate-900 items-center justify-center p-12 relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-md text-center">
          ...
          <p className="text-sm text-slate-400 leading-relaxed">
            Данные сайта и приложения Carvix полностью синхронизированы через единый API.
            Любые изменения отображаются в обоих интерфейсах в реальном времени.
          </p>
          <div className="mt-10 grid grid-cols-3 gap-4">
            {[
              { value: 'API', label: 'FastAPI бэкенд' },
              { value: 'SQLite', label: 'Единая БД' },
              { value: 'JWT', label: 'Авторизация' },
            ].map((s) => (
