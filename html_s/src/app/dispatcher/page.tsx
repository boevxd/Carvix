...
import { Truck, Wrench, Clock, CheckCircle2, AlertTriangle, Plus, ChevronRight, Loader2, Users } from 'lucide-react';
...
export default function DispatcherDashboard() {
  ...
  const { data: allRequests, loading: reqLoading, refetch: refetchReqs } = useApi(() => apiRequests.list({ limit: 10 }), []);
  const { data: vehicleList, loading: vehLoading } = useApi(() => apiVehicles.list({ limit: 8 }), []);
  const { data: maintenanceList, loading: maintLoading } = useApi(() => apiMaintenance.list({ status: 'Запланировано', limit: 5 }), []);

  const newRequests = allRequests?.filter(r => r.status === 'Новая') ?? [];

  return (
    <>
      <Header title=Диспетчерская subtitle={Управление заявками — } />
      ...
      <div className=p-4 lg:p-8 space-y-6>
        ...
        <div className=grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4>
          ...
          <MetricCard
            title=В работе
            value={stats?.requests.in_progress ?? '—'}
          ...
          <MetricCard
            title=Плановое ТО
            value={stats?.maintenance.planned ?? '—'}
                    ...
                    <div key={m.id} className=px-5 py-3.5>
                      <div className=flex items-center justify-between>
                        ...
                        <div className=text-right>
                          <span className={	ext-[11px] px-2 py-0.5 rounded-full border font-medium }>{m.status}</span>
                  ...
                  <div key={v.id} className=px-5 py-3 hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors>
                    <div className=flex items-center justify-between>
                      <div>
                        ...
                        {v.driver_name && <p className=text-[11px] text-slate-400>{v.driver_name}</p>}
                      </div>
                      <span className={	ext-[11px] px-2 py-0.5 rounded-full border font-medium }>{v.status}</span>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <Card>
                ...
                <div className=space-y-2>
                  {[
                    ...
                    { label: 'Запчасти (мало)', val: stats.parts.low_stock, color: stats.parts.low_stock > 0 ? 'text-red-600' : 'text-slate-600' },
                  ].map(item => (
                    <div key={item.label} className=flex items-center justify-between>
                      <span className=text-sm text-slate-500>{item.label}</span>
                      <span className={	ext-sm font-bold }>{item.val}</span>
                    </div>
                  ))}
                </div>
              </Card>
