...
import { Truck, Wrench, Bell, AlertTriangle, CheckCircle2, Clock, ChevronRight, Plus, Loader2 } from 'lucide-react';
import { Header } from '@/components/layout/header';
import { Card, Badge, Button, MetricCard } from '@/components/ui';
import { useAuth } from '@/contexts/auth-context';
import { requests as apiRequests, notifications as apiNotifications, vehicles as apiVehicles, analytics, ApiRequest, ApiNotification, ApiVehicle, DashboardAnalytics } from '@/lib/api';
...
export default function DriverDashboard() {
  ...
  return (
    <>
      <Header title={Добро пожаловать, } subtitle=Обзор состояния и последние события />
      ...
      <div className=p-4 lg:p-8 space-y-6>
        ...
        <div className=grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4>
          ...
          <MetricCard
            title=В работе
            value={myRequests?.filter(r => r.status === 'В работе').length ?? '—'}
            icon={<Clock size={20} className=text-amber-600 />}
            iconBg=bg-amber-50 dark:bg-amber-900/20
          />
          <MetricCard
            title=Уведомления
            value={myNotifications?.filter(n => !n.is_read).length ?? '—'}
            ...
            {myVehicle && (
              <Card>
                <div className=flex items-start justify-between mb-4>
                  ...
                  <span className={	ext-xs px-2 py-1 rounded-full border font-medium }>{myVehicle.status}</span>
                </div>
                  ...
                  {myRequests?.map((req) => (
                    <div key={req.id} className=flex items-center justify-between px-5 py-3.5 hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors>
                      ...
                      <div className=text-right flex-shrink-0 ml-4>
                        <span className={inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-medium border }>{req.priority}</span>
          ...
          <div className=space-y-6>
                ...
                <div className=space-y-3>
                  {[
                    ...
                    <div key={item.label} className=flex items-center justify-between>
                      ...
                      <span className={	ext-sm font-bold }>{item.value}</span>
                    </div>
                  ))}
                </div>
              </Card>
            ...
            <Card padding=none>
              <div className=flex items-center justify-between px-5 pt-5 pb-3>
                ...
                <Link href=/dashboard/notifications className=text-xs font-medium text-brand-600 flex items-center gap-1>
                  Все <ChevronRight size={14} />
                </Link>
              </div>
              {notifLoading ? (
                <div className=p-8 flex justify-center><Loader2 size={16} className=animate-spin text-slate-400 /></div>
                  ...
                  {myNotifications?.slice(0, 5).map((notif) => (
                    <div key={notif.id} className=px-5 py-3.5>
                      <div className=flex items-start gap-3>
                        ...
                        <div>
                          <p className={	ext-sm font-medium }>{notif.title}</p>
